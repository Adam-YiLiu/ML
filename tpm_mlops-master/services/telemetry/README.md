# Daemonised Telemetry (using Rust)
...
2. Save the following script in telemetry/src/main.rs
...
3. Build the Rust Project Binaries

```sh
cargo build --release
```

4. Copy the Rust release binary to /usr/local/bin/telemetry

```sh
sudo cp target/release/telemetry /usr/local/bin/telemetry
```

## Telemetry systemd Service

Save the following service in /etc/systemd/system/telemetry.service
...
2. Run `sudo systemctl start telemetry.service`
